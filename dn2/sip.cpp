...
void PoizvedbeZGlavo::sprintaj_vozlisca() {
    for (int k = 0; k < n_vozlisc; k++){
        cout << "(" << vozlisca[k].first << ", "<<vozlisca[k].second << ") ";
    }
    cout << "\n";
...
void PoizvedbeZGlavo::povecaj(int i, int j, int x){
    ...
    // povecaj alfe
    vozlisca[i_levo].first += x; // razlika na levem robu se poveča za x
    if (i_levo != i_desno){
        vozlisca[i_desno].first += j; // razlika na desnem robu se poveča za j
    }
    for (auto const & koren : notranja_vozlisca){
        vozlisca[koren].first += 1; // razlika se poveča za 1
    }
    // posodobi bete
    for (auto const & koren : notranja_vozlisca){
        posodobi_beta(koren);
...
void VsotaZGlavo::posodobi_beta(int i){
    if (je_notranje(i)){
        ...
        vozlisca[i].second = std::max(l_beta, d_beta) + alfa * n_listov_poddrevo(i);
    } else{
        vozlisca[i].second = 1;
        //cout << vozlisca[i].first << endl;
    }
}
...
void VsotaZGlavo::odgovor_vzdolz_ene_poti(int i, vector<int> & pot, vector<int> & vrednost_levo_desno, vector<int> & vozlisca_levo_desno){
    ...
    for (int j = 0; j < n; j++){
        ...
        // cout << "vr"  << i <<  " " << vrednost_levo_desno[i] << endl;
    }
}
